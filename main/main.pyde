...
def setup():
    ...
    lastGenBest = navigator.navigator(player.player(450, 180), random.randint(1, 15), random.randint(1, 15), random.randint(1, 60), tracks[nPista], random.randint(30, 90), random.randint(30, 90), random.randint(30, 90))
    ...
    if(trainingMode != 0):
        ...
        frameRate(30)
def draw():
    global pista, nPista, player, trainingMode, loops, car, ai, candidate_number, initial_x, initial_y, best, goal, running, tracks, lastGenBest
    if trainingMode == 1:
        track.desenha(pista, tracks[nPista])
...
#crossover
def crossover():
    global loops, ai, nPista, ran, runTimes, lastGenBest
    newAi = []
    with open("gn.txt", "a") as gn:
        for candidate in ai:
            ...
            if candidate != best: #(candidate.desiredSpeed != best.desiredSpeed or candidate.turningSpeed != best.turningSpeed or candidate.reverseDelay != best.reverseDelay or candidate.middleSensorDistance != best.middleSensorDistance or candidate.cornerSensorsDistance != best.cornerSensorsDistance or candidate.middleSensorDistance != best.middleSensorDistance or candidate.sideSensorsDistance != best.sideSensorsDistance):
        ...
        if(loops >= geracoes):
            ...
            with open("Fitness.txt", "r") as fit:
                with open("gn.txt", "r") as gn:
                    ...
                    #print(results)
                    if tracks[nPista] == 0:
                        with open ("resultados/teste2Running_fitness_pista0.txt", "a") as r:
                            r.write("melhor e media\n\n")
                            r.write(results)
                            r.write("\n\n")
                        with open ("resultados/teste2Running_gn_pista0.txt", "a") as gen:
                            gen.write("teste\n\n")
                            gen.write(generations)
                            gen.write("\n")
                    if tracks[nPista] == 1:
                        with open ("resultados/teste2Running_fitness_pista1.txt", "a") as r:
                            r.write("melhor e media\n")
                            r.write(results)
                            r.write("\n")
                        with open ("resultados/teste2Running_gn_pista1.txt", "a") as gen:
                            gen.write("teste\n\n")
                            gen.write(generations)
                            gen.write("\n")
                    if tracks[nPista] == 2:
                        with open ("resultados/testeaaaRunning_fitness_pista2.txt", "a") as r:
                            r.write("melhor e media\n\n")
                            r.write(results)
                            r.write("\n\n")
                        with open ("resultados/testeaaaRunning_gn_pista2.txt", "a") as gen:
                            gen.write("teste\n")
                            gen.write(generations)
                            gen.write("\n\n")
            if ran < runTimes:
