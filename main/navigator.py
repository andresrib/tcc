...
class navigator():
    ...
    def finishTrack(self):
        if(self.track == 0):
            ...
            if(self.previousX>450 and self.npc.x <= 450 and self.npc.y > 380):
                self.middlePoint = True
                #exit()
            if(self.previousY>210 and self.npc.y <= 210 and self.npc.x > 380 and self.npc.x < 500 and self.middlePoint):
                self.laps = self.laps + 1
                self.middlePoint = False
                #exit()
