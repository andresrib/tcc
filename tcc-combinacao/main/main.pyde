...
def draw():
    ...
    if trainingMode == 1:
        ...
        with open("Fitness.txt", "a") as fit:
            average = 0
            #best = candidate.get(0)
            for candidate in ai:
                average += candidate.fitness
                if(candidate.fitness <= best.fitness):
                    best = candidate#[candidate.desiredSpeed, candidate.turningSpeed, candidate.startingReverseDelay, candidate.fitness]
            #print(best)
            fit.write(str(best.fitness) + "\t"  + str(average/60) + "\n")
            newAi = []
            with open("gn.txt", "a") as gn:
                for candidate in ai:
                    gn.write(str(candidate.desiredSpeed) + " " + str(candidate.turningSpeed) + " " + str(candidate.startingReverseDelay) + " " + str(candidate.fitness) + "\n")
                ...
                if(loops >= 49):
                    ...
                    with open("Fitness.txt", "r") as fit:
                        with open("gn.txt", "r") as gn:
                            ...
                            #print(results)
                            if nPista == 0:
                                with open ("resultados/v2_combinacross_fitness_pista0.txt", "a") as r:
                                    r.write("melhor e media\n\n")
                                    r.write(results)
                                    r.write("\n\n")
                                with open ("resultados/v2_combinacross_gn_pista0.txt", "a") as gen:
                                    gen.write("teste\n\n")
                                    gen.write(generations)
                                    gen.write("\n")
                            if nPista == 1:
                                with open ("resultados/v2_combinacross_fitness_pista1.txt", "a") as r:
                                    r.write("melhor e media\n")
                                    r.write(results)
                                    r.write("\n")
                                with open ("resultados/v2_combinacross_gn_pista1.txt", "a") as gen:
                                    gen.write("teste\n\n")
                                    gen.write(generations)
                                    gen.write("\n")
                            if nPista == 2:
                                with open ("resultados/v2H_combinacross_fitness_pista2.txt", "a") as r:
                                    r.write("melhor e media\n\n")
                                    r.write(results)
                                    r.write("\n\n")
                                with open ("resultados/v2H_combinacross_gn_pista2.txt", "a") as gen:
                                    gen.write("teste\n")
                                    gen.write(generations)
                                    gen.write("\n\n")
                    exit()
